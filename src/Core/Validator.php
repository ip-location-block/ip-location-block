...
class Validator {
	...
	private static function _get_geolocation( $ip, $settings, $providers, $args = array(), $callback = '' ) {
		...
		foreach ( (array) $providers as $provider ) {
			...
			$instance = \IPLocationBlock\Providers\ProviderRegistry::instance()->get( $provider );
			if ( $instance ) {
				$instances[] = $instance;
			}
		}

		// Build the per-lookup context. get_request_headers() is already
		// 'ip-location-block-headers'-filtered; $args may carry the legacy
		// 'cache' / 'asn' / 'fresh' control keys (fromLegacyArgs lifts asn/fresh
		// into typed flags and drops them from the HTTP args).
		...
		return self::make_validation( $ip, $result );
	}
	...
	public static function validate_country( $hook, $validate, $settings, $block = true ) {
		\IPLocationBlock\Compat::deprecated( 'IP_Location_Block::validate_country', '1.2.0', 'IP_Location_Block::validate_lookup_result' );
	...
	public function send_response( $hook, $validate, $settings ) {
		...
		\nocache_headers(); // wp-includes/functions.php @since  0.2.0.0
	...
	public function validate_public() {
		...
		// Optionally prevent PHP-level cache plugins (WP Super Cache, W3TC, WP Rocket,
		// LiteSpeed, WP-Optimize) from serving a single cached response to all visitors.
		// Opt-in: this disables full-page caching, so it is off by default.
		if ( ( 1 & $settings['validation']['public'] ) && ! empty( $settings['public']['cache_bypass'] ) ) {
			\defined( 'DONOTCACHEPAGE' ) or \define( 'DONOTCACHEPAGE', true );
		}
	}
