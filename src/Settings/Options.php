...
class Options {
	...
	public static function get_legacy_settings( $mutate = true ) {
		...
		if ( version_compare( $version, '3.0.11' ) < 0 ) {
			...
			if ( $mutate ) {
				Logs::delete_tables( 'ip_geo_block_cache' );
			}
		}
	...
	public static function setup_validation_timing( $settings = null, $prefix = '' ) {
		...
		switch ( $settings ? (int) $settings['validation']['timing'] : 0 ) {
			...
			case 1: // mu-plugins
				$src = Util::slashit( IP_LOCATION_BLOCK_PATH ) . 'wp-content/mu-plugins/ip-location-block-mu.php';
				$dst = Util::slashit( WPMU_PLUGIN_DIR ) . $prefix . 'ip-location-block-mu.php';
