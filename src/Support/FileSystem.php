	...
	public static function init( $msg = null ) {
		...
		if ( empty( $wp_filesystem ) ) {
			if ( 0 ) {
				...
				if ( 'direct' === ( self::$method = get_filesystem_method() ) ) { // @since  0.2.5.0
					// request_filesystem_credentials() can be run without any issues and don't need to worry about passing in a URL
					$creds = request_filesystem_credentials( admin_url(), '', false, false, null ); // @since  0.2.5.0
				...
				// admin (IP_Location_Block_Admin) exists only under its legacy name.
					...
					\IP_Location_Block_Admin::add_admin_notice(
						'error',
						sprintf( __( 'This plugin does not support method &#8220;%s&#8221; for FTP or SSH based file operations. Please refer to <a href="https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants" title="Editing wp-config.php &laquo; WordPress Codex">this document</a> for more details.', 'ip-location-block' ), self::$method )
					);
				}
			} else {
				...
				if ( false !== ( $creds = request_filesystem_credentials( admin_url(), '', false, false, null ) ) ) {
					...
					\IP_Location_Block_Admin::add_admin_notice(
						'error',
						__( 'You should define some constants in your <code>wp-config.php</code> for FTP or SSH based file operations. Please refer to <a href="https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants" title="Editing wp-config.php &laquo; WordPress Codex">this document</a> for more details.', 'ip-location-block' )
					);
				}
			}
		}
